# Orchestrator — the complete guide

Everything the [README](../README.md) keeps short, in full: how Orchestrator
compares to its neighbors, every feature in depth, the complete
configuration reference, security posture, FAQ, and the honest
limitations. Agents driving Orchestrator should read
[AGENT_GUIDE.md](AGENT_GUIDE.md) (the machine contract) first; humans
start here.

## How it compares

|  | juggling CLIs by hand | Claude Code agent teams (native) | **orchestrator** |
|---|---|---|---|
...
| Parallel write safety | you, by discipline | in-session | a git worktree per teammate + `orchestrator land` — collisions physically impossible, work always reconciled back |
...
multiple agent sessions in tmux — Orchestrator's center of gravity is
...
Claude-driven tmux workflows — Orchestrator is provider-neutral plumbing with
real usage tracking and verified teardown. **Claude Code's native agent
teams** directly inspired Orchestrator (see the README credits) and remain
the deepest Claude-native experience; Orchestrator reimagines the shape on
...
reboot, `orchestrator resurrect` rebuilds the roster from what Orchestrator
...
```sh
orchestrator update --check     # installed vs latest, from your install source
orchestrator update             # replace ~/.local/bin/orchestrator
```

The installer records where Orchestrator came from (`install-meta.json` in the
...
Orchestrator also runs a **background version check** at most once a day
...
`providers`/`usage`:
    ...
    · orchestrator X.Y.Z available — orchestrator update
...
re-run `install.sh` once and `orchestrator update` works from then on.
...
recorded (so `orchestrator update` works), a writable state dir, orphaned
worktrees, and crash-lost teammates waiting for `orchestrator resurrect`. The
exit code reflects the worst finding (`0` ok · `1` warn · `2` fail), so
it drops into CI or a pre-flight script.

## Provider states

Every surface that names a provider — `orchestrator providers`, `Orchestrator
...
Orchestrator says so instead of inventing numbers.) A custom provider with no
...
- [tmux](https://github.com/tmux/tmux) (Orchestrator runs inside a tmux session)
...
```sh
orchestrator        # ← just this
```

Type `orchestrator` on its own in a terminal and you're in: it opens your
default chat provider as the **lead** — in the `orchestrator` tmux session, with
...
the only signed-in provider); set them in `orchestrator settings`. If nothing's
signed in, or the chosen provider is locked out, it says so and names the
fix — never a stack trace. Piped or in CI (no terminal), bare `orchestrator`
prints usage as before. And if a reboot took your roster with it, the
front door says so and offers `orchestrator resurrect`.
...
gets its own branch (`orchestrator/<role>`) checked out in its own worktree
...
repo Orchestrator says so once and uses the plain cwd.

**Landing** closes the loop:

```sh
orchestrator land <role> --dry-run   # diffstat, commit/dirty counts, target branch
orchestrator land <role>             # checkpoint uncommitted work (asks first)
...
the worktree registry survives reboots. `orchestrator worktree list` audits
everything Orchestrator ever created; `worktree prune` removes only what
provably holds nothing (never a live teammate's, never unique work).
Under the hood Orchestrator only ever uses git's own *non-forced* commands
...
the same sources `orchestrator usage` shows (native feeds first, probe cache
...
`orchestrator config routing.strategy preference` restores the strict-order
behavior, bit-for-bit.

## Status: who's busy, who's blocked, who's idle

```sh
orchestrator status                 # one-shot table
orchestrator status --watch         # poll; prints transitions until Ctrl-C
orchestrator status --json          # for an agent lead
```
...
```sh
orchestrator resurrect --dry-run    # what was lost, what a rebuild would do
orchestrator resurrect              # rebuild (asks once; --yes for scripts)
```

Interactive teammates respawn **fresh** with everything Orchestrator recorded
...
conversation context was *not* restored — Orchestrator never captured
...
reboot. Bare `orchestrator`, `list`, and `doctor` all point out lost teammates;
...
each further teammate splits the largest non-lead pane (Orchestrator-spawned or
...
with the optional tmux block from `orchestrator init` every pane border shows
...
`done` (`orchestrator list` shows lifecycle state), `result` keeps working
...
**`orchestrator lead on` automates the paste-in for every detected CLI** (see
...
anything large:
1. Run `orchestrator usage` — real usage %/reset times where the provider exposes
   them (it also refreshes availability signals as a side effect).
2. Run `orchestrator providers` — which subscriptions are installed, authed, and
...
3. Decide the provider from that live data plus task fit — or use
   `orchestrator route` to auto-pick (most headroom wins) and dispatch in one
...
4. For a live interactive teammate you can also send `/usage` into its pane
   (`orchestrator send <role> "/usage"`, then tmux capture-pane) to read that
...
5. Match model and effort to the task — light/cheap models for mechanical
   ...
   (`orchestrator land <role>`), and shut every teammate down
   (`orchestrator shutdown <role>`) the moment its job is done.
```

## Lead mode

`orchestrator lead on` installs a durable *manager identity* for your lead
agent — the standing rules from the playbook above (stay responsive,
delegate non-trivial work, decide capacity from `orchestrator usage`/`providers`
...
2. **Skill** (`~/.claude/skills/teamctl-lead/SKILL.md`) — *discoverable;
   ...
   settings menu ("open the Orchestrator menu").
3. **Hook** (recommended; asked as Y/n — default **yes** — or forced with
   `orchestrator lead on --hook`) — *per-prompt, the strongest tier; Claude Code
...
changes an existing file, and printed with its exact revert. `Orchestrator lead
...
- **ask** (default): the first time non-trivial work comes up in a
  ...
  (`orchestrator config lead.delegation always|manual`). Once, then never
  again that session.
...
**The off switch:** `orchestrator lead off` removes exactly what `on` installed —
...
`orchestrator lead status` shows per-tier, per-CLI state at any time.

## Configuration

> **`orchestrator init` is a twelve-line dark room that finds your providers and
...
- **`orchestrator init` — express (the default).** Zero questions: detects your
...
- **`orchestrator init --custom` — the cockpit.** A three-screen arrow-key
...
- **The plain path.** Wherever the cockpit can't run — no tty, dumb TERM,
  ...
  live on the seal screen and in `orchestrator lead on`;
...
[providers.gemini]
model = "gemini-2.5-pro"    # gemini has no effort flag — orchestrator never
...
[notify]
# command = "notify-send orchestrator"  # run on observed status transitions;
...
including panes Orchestrator didn't create — instead of carving slivers off the
lead.
...
**`orchestrator settings`** is the human face: a re-runnable dark-room cockpit
...
values plus the matching `orchestrator config` one-liner for each — the
scriptable path spelled out.

For scripting or one-off tweaks, `orchestrator config` is the direct path (and
never needs the TOML edited by hand):

```sh
orchestrator config                                  # show current settings as dotted keys
orchestrator config providers.claude.model           # show one value
orchestrator config providers.claude.model sonnet    # set one key (others preserved)
orchestrator config routing.preference "codex,claude"  # comma-separated -> list
orchestrator settings                                # the full cockpit (or config --menu)
```

Or from a chat: with [lead mode](#lead-mode) on, tell your lead agent
*"open the Orchestrator menu"* — the teamctl-lead skill teaches it to read your
settings with `orchestrator config`, present them as a numbered menu in chat,
and apply your changes key by key (and to point you at `orchestrator settings`
for the full cockpit).

### Models

Model ids pass through to the provider CLI **verbatim** — Orchestrator carries
no model list, so new models work the day a provider ships them, with no
Orchestrator update. `orchestrator models [provider]` is best-effort discovery for
...
`orchestrator usage --probe [provider|all]` opens a provider's own TUI in a
...
are cached with a timestamp; plain `orchestrator usage` reports them alongside
...
their TUI — quiet, honestly. Probing is always explicit — Orchestrator never
...
`config.toml.bak-orchestrator`, all other keys are preserved, and a config that
fails to parse is refused rather than silently replaced. You can also just
re-run `orchestrator init` any time to redo the whole wizard.

`orchestrator init` also offers (default **no**, backups made first):
...
- wiring Claude Code's `statusLine` in `~/.claude/settings.json` to
  `orchestrator statusline` (shows `model · effort · ctx N%`) — skipped if a
  non-Orchestrator `statusLine` key already exists, and a pre-v0.4.0
  ...
  which is what powers `orchestrator usage`'s Claude column;
- installing [lead mode](#lead-mode) for your detected agent CLIs (same as
  `orchestrator lead on`).

## Uninstall

```sh
orchestrator uninstall     # add --yes to skip the confirm
```

Removes the `orchestrator` binary (and any pre-v0.4.0 `claude-statusline`
companion), the `~/.tmux.conf` marker block, the `statusLine` settings key
if — and only if — it points at Orchestrator, and the install/update metadata —
each with a backup first. Config and state files are left in place; the
one-liner to remove them too is printed.

If you installed [lead mode](#lead-mode), run `orchestrator lead off` **before**
uninstalling (the uninstaller removes the `orchestrator` binary itself, so it
...
`--approval-mode`). **Orchestrator itself never sets those flags** — each
...
Also remember `orchestrator send` types raw keys into a live agent's terminal —
anything (or anyone) able to run Orchestrator can steer every teammate.

**The threat model, named:** every teammate runs a vendor CLI
autonomously with `orchestrator` on its PATH and shared team state — so a
prompt-injected teammate is not just a bad worker, it's an *actor* that
could run Orchestrator against its siblings (spawn, send keys, shut down).
...
**Where Orchestrator is deliberately boring:** it never reads, stores, or
...
using teams, and a no is final). `orchestrator usage` shows real meters before
...
probes (`usage --probe`) are always explicit — Orchestrator never opens a
provider session on its own.

**What if I want out?**
`orchestrator uninstall` — removes the binary and every integration it added
(backups first), and prints the one-liner to remove config/state too. If
you enabled lead mode, `orchestrator lead off` first; it removes exactly what
...
isolation". Orchestrator adds no autonomous flags itself; each teammate
...
with the Orchestrator statusline); Grok exposes nothing locally (the explicit
...
locations, probe TUI text) — Orchestrator parses them defensively and
...
exact `orchestrator land` command to reconcile it — which works even after
...
`orchestrator resurrect` rebuilds the roster after a reboot. Border labels
...
- **Usage numbers exist only where providers expose them locally.** Codex
  writes rate-limit windows to its session logs, so `orchestrator usage` shows real
  ...
  after a Claude Code turn has run with the Orchestrator statusline installed,
  and `orchestrator usage` labels the cache's age. Grok exposes nothing locally
  (the hidden probe fills it in on request); Gemini exposes nothing at all
  — `orchestrator usage` says so rather than inventing numbers, and headroom
...
- **Gemini resume has a known upstream flake.** Exact-session follow-ups
  ...
  follow-ups run from the dispatch cwd (Orchestrator already guarantees this).
- **Worktrees cost disk and don't share untracked files.** A teammate's
  ...
  inherent to the isolation. `orchestrator worktree list` shows everything;
...
- **Some parsed provider formats are observed, not documented.** Grok's JSON
  ...
  output. Orchestrator parses them all defensively and degrades to "usage
...
- Provider CLIs must already be installed and logged in; Orchestrator never
  ...
  `auth_env` you configure — and with neither, Orchestrator says "auth not
