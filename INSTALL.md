# Installing teamctl — instructions for your AI agent

You are an agent installing teamctl for your user. teamctl runs AI
...
2. **Configure**: run `teamctl init` (express — zero questions) and show
   the user its summary frame. Never pick a routing order for them.

3. **Offer lead mode**: `teamctl lead on` installs a manager identity
   into every detected agent CLI (reversible: `teamctl lead off`).
   Install it only on a yes.

4. **Verify**: run `teamctl doctor` and show the output
   (exit 0 ok · 1 warn · 2 fail). `teamctl providers` shows what is
   signed in.
...
6. **Report** what you did, what you skipped and why, and the user's
   controls: `teamctl settings` (preferences cockpit) ·
   `teamctl lead on|off|status` (manager identity) ·
   `teamctl uninstall` (run `teamctl lead off` first — the uninstaller
